    ...
    for schema_item in RoutesSchema:
        ...
        if options.routes and options.routes.exclude:
            if router_name in options.routes.exclude:
                continue
        exist_overrides = list(filter(lambda route: route.path==schema_item["path"] and schema_item["method"].upper() in route.methods, router.routes))
        if exist_overrides:
            continue
        if schema_item.get("summary"):
            summary = schema_item["summary"].replace("{name}",options.name if options.name else "")
        else:
            summary = None
